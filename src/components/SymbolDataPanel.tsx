...
export function SymbolDataPanel() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [tickerRes, fundingRes, oiRes, lsRes, depthRes] =
          await Promise.all([
            ...
            okxPublicFetch(
              `/api/v5/rubik/stat/contracts/long-short-account-ratio?ccy=${ccy}`,
            ),
        ...
        if (active) {
          ...
          const oi = oiRes?.[0] || {};
          const lsData =
            Array.isArray(lsRes) && lsRes.length > 0 ? lsRes[0] || [] : [];
          ...
          if (depth && depth.bids && depth.asks) {
            const bidVol = depth.bids.reduce(
              (acc: number, curr: any[]) => acc + parseFloat(curr[1]),
              0,
            );
            const askVol = depth.asks.reduce(
              (acc: number, curr: any[]) => acc + parseFloat(curr[1]),
              0,
            );
  ...
  return (
    <div className="shrink-0 bg-[#161a1e] border border-[#2b2f36] rounded-lg p-3 flex flex-col gap-3 overflow-hidden">
      <div className="flex items-center gap-2 mb-1 shrink-0">
        ...
        <h2 className="text-sm font-bold tracking-wider flex items-center justify-between w-full">
          <span className="flex items-center gap-2">市场情绪数据</span>
          <span className="text-xs font-mono text-gray-500 bg-[#1e2329] px-2 py-0.5 rounded border border-[#2b2f36] shadow-sm">
            {activeMainSymbol.replace("-SWAP", "")}
          </span>
        </h2>
      </div>

      <div className="space-y-3 pr-1 flex flex-col justify-start pb-2">
        {loading && !data ? (
          ...
          <>
            <div className="bg-[#1e2329] rounded p-3 flex flex-col gap-2 relative overflow-hidden border border-[#2b2f36] shadow-inner">
              <div className="flex justify-between items-start">
                <div className="flex flex-col">
                  <span className="text-[10px] text-gray-400 flex items-center gap-1 uppercase tracking-wider mb-1">
                    最新价
                  </span>
                  <span
                    className={`text-xl font-mono block font-bold ${parseFloat(data.change24hPct) >= 0 ? "text-green-400" : "text-red-400"}`}
                  >
                    {formatNumber(
                      data.lastPrice,
                      parseFloat(data.lastPrice) < 1 ? 4 : 2,
                    )}
                  </span>
                </div>
                <div className="flex flex-col items-end text-right">
                  ...
                  <span
                    className={`text-sm font-mono font-bold ${parseFloat(data.change24hPct) >= 0 ? "text-green-400" : "text-red-400"}`}
                  >
                    {parseFloat(data.change24hPct) > 0 ? "+" : ""}
                    {parseFloat(data.change24hPct).toFixed(2)}%
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-[#1e2329] border border-[#2b2f36] rounded p-2 flex flex-col gap-1 transition-colors hover:bg-[#252a32]">
                ...
                <span
                  className={`text-sm font-mono font-bold ${getFundingRateColor(data.fundingRate)}`}
                >
              ...
              <div className="bg-[#1e2329] border border-[#2b2f36] rounded p-2 flex flex-col gap-1 transition-colors hover:bg-[#252a32]">
                ...
                <span
                  className={`text-sm font-mono font-bold ${parseFloat(data.longShortRatio) > 1 ? "text-green-400" : "text-red-400"}`}
                >
                  {data.longShortRatio !== "N/A"
                    ? parseFloat(data.longShortRatio).toFixed(2)
                    : "N/A"}
                </span>
            ...
            <div className="grid grid-cols-2 gap-2 mt-1">
              <div className="bg-[#1e2329] border border-[#2b2f36] rounded p-2 flex flex-col gap-0.5">
                ...
                <span className="text-sm font-mono font-bold text-gray-200">
                  {formatNumber(data.volCcy24h)}{" "}
                  <span className="text-[9px] font-normal text-gray-500">
                    {activeMainSymbol.split("-")[0]}
                  </span>
                </span>
              </div>
